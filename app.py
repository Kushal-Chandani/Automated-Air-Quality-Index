...
def load_model():
    ...
    try:
        ...
        HOPSWORKS_API_KEY = "2EpVtPZvfyir2ZHe.Xq5Zf52NZvrcFMazBANKnavDajjwl759POapcm1FijsZhoDFqhKeY2zu331fo82i" # Replace with your key or use secrets
...
def main():
    ...
    with st.sidebar:
        ...
        last_hist_update_str = latest_data['date'].strftime('%Y-%m-%d %H:%M:%S %Z') if latest_data is not None else "N/A"
        st.write(f"Data timestamp: {last_hist_update_str}")
        st.write(f"App refresh: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
